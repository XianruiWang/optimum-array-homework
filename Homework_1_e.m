%--------------------------------------------------------------------------
% Homework of array signal processing 1.e
% 
% Inputs:
...
signalPara.steeringVec = exp(-1j*arrayPara.phaseLag*arrayPara.vec*...
    cos(signalPara.doa));
%% Weight vector
if arrayPara.weight == 'dc'
    attenuFac=20; % attenuation 20dB in sidelobs
    h=chebwin(arrayPara.M,attenuFac); % DC weighting
    h=(h/sum(h)).';
end
...
for snrIndex = 1:length(signalPara.snr)
    ...
    for sourceIndex=1:signalPara.num
        ...
        noiseRes = h*noise;
        receivedSig = d*signalPara.sourceSignal.';
        outputSignal.power(snrIndex,sourceIndex) = 10*log10((h*receivedSig)*...
            (h*receivedSig)'+noiseRes*noiseRes');
    end
end
%% Figure plot
angle = 0:180;
